...
namespace SPEngine
{
	...
	//-----------------------------------------------------------------------------
	VideoRenderer2::VideoRenderer2(VideoTexture2* setVideo, LPUNKNOWN pUnk, HRESULT *phr )
	...
	//-----------------------------------------------------------------------------
	HRESULT VideoRenderer2::CheckMediaType(const CMediaType *pmt)
	{
		HRESULT   hr = E_FAIL;
		VIDEOINFO *pvi = NULL;
	...
	//-----------------------------------------------------------------------------
	HRESULT VideoRenderer2::SetMediaType(const CMediaType *pmt)
	{
		video->g_Lock.Lock();		
	...
	//-----------------------------------------------------------------------------
	HRESULT VideoRenderer2::DoRenderSample( IMediaSample * pSample )
	{
		video->g_Lock.Lock();
		...
		// Unlock the Texture
		if (FAILED(video->texture->UnlockRect(0)))
			return E_FAIL;
